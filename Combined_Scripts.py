...
class Script_3:
    ...
    def plotDRC_new(self, xdata,ydata, std, b, c, d, e, err = None, IC50lines=True,
        ...
        ax.errorbar(xdata, ydata, yerr=std, linestyle="None", capsize=3, ecolor = color)
        ax.plot(np.ndarray.flatten(x),np.ndarray.flatten(y), label= None, color = color)
        ax.plot(np.ndarray.flatten(xdata), np.ndarray.flatten(ydata), 'o', label='data points for' + ' ' + drug, color = color) #plotting of data #plotting of curve
